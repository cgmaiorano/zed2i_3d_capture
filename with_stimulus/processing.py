...
def body_tracking(sharedstate):
    body_param, body_runtime_param = zed_parameters.initialize_tracking_parameters(
        sharedstate.zed
    )
    ...
    while True:
        ...
        if sharedstate.zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            for i, part in enumerate(sl.BODY_38_PARTS):
                if i < 38:
                    ...
                    kp = bodies.body_list[
                        ...
                        i
                    ]  # get 1x3 array of current body part in x, y, z. bodies.body_list[0].keypoint: 38x3 array of xyz coordinates of all body parts
                    row.append(kp[0])
                    row.append(kp[1])
                    row.append(kp[2])
                else:
            ...
            # Display skeletons
            sharedstate.zed.retrieve_image(
